...
export const Editar = () => {
  ...
  const getArticulo = async () => {
    const { datos } = await Request(
      global.url + "articulo/" + params.edit,
      "GET"
    );
  ...
  const editarArticulo = async (e) => {
    ...
    if (datos.status === "success" && fileInput.files[0]) {
      ...
      const subirImagen = await Request(
        global.url + "subir-image/" + params.edit,
        "post",
        formData,
        true
      );
  ...
  return (
    <div className="create">
      ...
      <p className="create__description">
        Formulario para editar: {Articulo.titulo}
      </p>

      <strong className="warn">
        {guardado === "Guardado" ? "Se ha guardado el articulo" : ""}
      </strong>
      <strong className="warn">
        {guardado === "error" ? "Faltan datos en el archivo" : ""}
      </strong>
      ...
      <form className="form" onSubmit={editarArticulo}>
        ...
        <div className="form__group">
          <label htmlFor="corno" className="form__label">
            Categorias
          </label>
          <input
            type="text"
            ...
            className="form__input"
            onChange={cambiar}
            defaultValue={Articulo.corno}
          />
        </div>

        <div className="form__group">
          <label htmlFor="file0" className="form__label">
            Subir Archivo
          </label>
