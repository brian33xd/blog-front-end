...
export const ListadoAdmin = ({ Articulos, setArticulos }) => {
  ...
  return Articulos.map((articulos) => {
    return (
      <article className="article__item" key={articulos._id}>
        <section className="article__mask">
          {articulos.image != "image.png" && (
            <img
              src={global.url + "imagen/" + articulos.image}
        ...
        <section className="article__content">
          <Link to={"/articulo/" + articulos._id} className="article__title">
            {articulos.titulo}
          </Link>
          <p className="article__description">{articulos.description}</p>

          <Link to={"/editar/" + articulos._id} className="article__edit">
            Edit
          </Link>
          <button
